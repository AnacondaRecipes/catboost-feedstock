...
if [[ "${gpu_variant}" == cuda* ]]; then
    ...
    # set(ENV{NVCC_PREPEND_FLAGS} ...); conda.patch drops that line so it doesn't
    ...
    if [[ "$cuda_compiler_version" != "None" ]]; then
        ...
        # hardcode the static names, so a global rewrite is necessary --
        # without this, the final link step fails with `unable to find library -lculibos`.
